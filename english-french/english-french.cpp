...
// syntax "left_word: right_word"
{
	...
	if (dictionary.is_open()) {
		...
		if (left_words.size() != right_words.size())
			throw runtime_error("(retrieve words) size mismatch.");
	}
	else
...
void display_messed_up_words(const size_t& score, const size_t& size, const vector<int>& messed_up, const vector<wstring>& left, const vector<wstring>& right) {
	if (score != size) {
		cout << "\nMessed up words:\n" << endl;
		for (unsigned i = 0; i < messed_up.size(); ++i)
			wcout << left[messed_up[i]] << L": " << right[messed_up[i]] << endl;
	}
}

// main quiz function
void vocabulary_quiz(const vector<wstring>& left, const vector<wstring>& right, vector<int>& messed_up, const string& practice_filename) {
	const char cperiod{ '$' };
...
int test_vocabulary(const vector<wstring>& left, const vector<wstring>& right, vector<int>& messed_up_indexes)
...
// keeps track of the failed words
{
	if (left.size() != right.size())
		throw runtime_error("(vocabulary quiz) size mismatch.");
...
void practice_vocabulary(const vector<wstring>& left, const vector<wstring>& right, vector<int>& messed_up)
{
